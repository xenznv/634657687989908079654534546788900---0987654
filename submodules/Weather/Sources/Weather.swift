...
public struct WeatherData {
    ...
    public init(emoji: String, temperature: Double) {
        self.emoji = emoji
...
public func getWeatherData(context: AccountContext, load: Bool) -> Signal<WeatherDataState, NoError> {
    if !load {
        ...
        case .allowed:
            ...
            |> then(
                ...
                |> mapToSignal { location in
                    ...
                    return requestWeatherData(context: context, location: location)
                    |> map { weather in
                        if let weather {
...
public func requestWeatherData(context: AccountContext, location: CLLocationCoordinate2D) -> Signal<WeatherData?, NoError> {
    ...
    if let botUsername = botConfiguration.botName {
        ...
        |> map { contextResult -> WeatherData? in
            ...
            let effectiveEmoji = emojiFor(for: strippedEmoji(emoji), date: Date(), location: location)
            return WeatherData(emoji: effectiveEmoji, temperature: temperature)
        }
    } else {
        return .single(nil)
    }
...
private func strippedEmoji(_ emoji: String) -> String {
    var string = ""
