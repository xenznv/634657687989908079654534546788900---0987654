...
CustomDcSctpSocket::CustomDcSctpSocket(absl::string_view log_prefix,
    ...
    : log_prefix_(std::string(log_prefix) + ": "),
      ...
      t1_init_(timer_manager_.CreateTimer(
          "t1-init",
          absl::bind_front(&CustomDcSctpSocket::OnInitTimerExpiry, this),
      ...
      t1_cookie_(timer_manager_.CreateTimer(
          "t1-cookie",
          absl::bind_front(&CustomDcSctpSocket::OnCookieTimerExpiry, this),
      ...
      t2_shutdown_(timer_manager_.CreateTimer(
          "t2-shutdown",
          absl::bind_front(&CustomDcSctpSocket::OnShutdownTimerExpiry, this),
          TimerOptions(options.t2_shutdown_timeout.ToTimeDelta(),
                       TimerBackoffAlgorithm::kExponential,
                       options.max_retransmissions))),
      packet_sender_(callbacks_,
                     absl::bind_front(&CustomDcSctpSocket::OnSentPacket, this)),
...
std::string CustomDcSctpSocket::log_prefix() const {
  return log_prefix_ + "[" + std::string(ToString(state_)) + "] ";
}

bool CustomDcSctpSocket::IsConsistent() const {
...
constexpr absl::string_view CustomDcSctpSocket::ToString(CustomDcSctpSocket::State state) {
  switch (state) {
    case CustomDcSctpSocket::State::kClosed:
      return "CLOSED";
    case CustomDcSctpSocket::State::kCookieWait:
      return "COOKIE_WAIT";
    case CustomDcSctpSocket::State::kCookieEchoed:
      return "COOKIE_ECHOED";
    case CustomDcSctpSocket::State::kEstablished:
      return "ESTABLISHED";
    case CustomDcSctpSocket::State::kShutdownPending:
      return "SHUTDOWN_PENDING";
    case CustomDcSctpSocket::State::kShutdownSent:
      return "SHUTDOWN_SENT";
    case CustomDcSctpSocket::State::kShutdownReceived:
...
void CustomDcSctpSocket::SendInit() {
...
void CustomDcSctpSocket::Connect() {
...
void CustomDcSctpSocket::CreateTransmissionControlBlock(
...
void CustomDcSctpSocket::RestoreFromState(const DcSctpSocketHandoverState& state) {
  CallbackDeferrer::ScopedDeferrer deferrer(callbacks_);
...
void CustomDcSctpSocket::Shutdown() {
  CallbackDeferrer::ScopedDeferrer deferrer(callbacks_);
...
void CustomDcSctpSocket::CloseConnectionBecauseOfTooManyTransmissionErrors() {
...
void CustomDcSctpSocket::InternalClose(ErrorKind error, absl::string_view message) {
  if (state_ != State::kClosed) {
    t1_init_->Stop();
    t1_cookie_->Stop();
    t2_shutdown_->Stop();
...
void CustomDcSctpSocket::SetStreamPriority(StreamID stream_id,
                                     StreamPriority priority) {
  send_queue_.SetStreamPriority(stream_id, priority);
}
StreamPriority CustomDcSctpSocket::GetStreamPriority(StreamID stream_id) const {
  return send_queue_.GetStreamPriority(stream_id);
}

SendStatus CustomDcSctpSocket::Send(DcSctpMessage message,
...
std::vector<SendStatus> CustomDcSctpSocket::SendMany(
...
SendStatus CustomDcSctpSocket::InternalSend(const DcSctpMessage& message,
                                      const SendOptions& send_options) {
...
ResetStreamsStatus CustomDcSctpSocket::ResetStreams(
...
SocketState CustomDcSctpSocket::state() const {
  switch (state_) {
    case State::kClosed:
...
size_t CustomDcSctpSocket::buffered_amount(StreamID stream_id) const {
  return send_queue_.buffered_amount(stream_id);
}

size_t CustomDcSctpSocket::buffered_amount_low_threshold(StreamID stream_id) const {
  return send_queue_.buffered_amount_low_threshold(stream_id);
}

void CustomDcSctpSocket::SetBufferedAmountLowThreshold(StreamID stream_id,
                                                 size_t bytes) {
  send_queue_.SetBufferedAmountLowThreshold(stream_id, bytes);
}

absl::optional<Metrics> CustomDcSctpSocket::GetMetrics() const {
  if (tcb_ == nullptr) {
    return absl::nullopt;
  }
...
void CustomDcSctpSocket::MaybeSendShutdownOnPacketReceived(const SctpPacket& packet) {
...
void CustomDcSctpSocket::MaybeSendResetStreamsRequest() {
...
bool CustomDcSctpSocket::ValidatePacket(const SctpPacket& packet) {
...
void CustomDcSctpSocket::HandleTimeout(TimeoutID timeout_id) {
  CallbackDeferrer::ScopedDeferrer deferrer(callbacks_);
...
void CustomDcSctpSocket::ReceivePacket(rtc::ArrayView<const uint8_t> data) {
  CallbackDeferrer::ScopedDeferrer deferrer(callbacks_);
...
void CustomDcSctpSocket::DebugPrintOutgoing(rtc::ArrayView<const uint8_t> payload) {
...
bool CustomDcSctpSocket::Dispatch(const CommonHeader& header,
                            const SctpPacket::ChunkDescriptor& descriptor) {
...
bool CustomDcSctpSocket::HandleUnrecognizedChunk(
    const SctpPacket::ChunkDescriptor& descriptor) {
...
TimeDelta CustomDcSctpSocket::OnInitTimerExpiry() {
...
TimeDelta CustomDcSctpSocket::OnCookieTimerExpiry() {
...
TimeDelta CustomDcSctpSocket::OnShutdownTimerExpiry() {
...
void CustomDcSctpSocket::OnSentPacket(rtc::ArrayView<const uint8_t> packet,
...
bool CustomDcSctpSocket::ValidateHasTCB() {
  if (tcb_ != nullptr) {
    return true;
  }
...
void CustomDcSctpSocket::ReportFailedToParseChunk(int chunk_type) {
  rtc::StringBuilder sb;
  sb << "Failed to parse chunk of type: " << chunk_type;
  callbacks_.OnError(ErrorKind::kParseFailed, sb.str());
}

void CustomDcSctpSocket::HandleData(const CommonHeader& header,
                              const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleIData(const CommonHeader& header,
                               const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleDataCommon(AnyDataChunk& chunk) {
  TSN tsn = chunk.tsn();
...
void CustomDcSctpSocket::HandleInit(const CommonHeader& header,
                              const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleInitAck(
...
void CustomDcSctpSocket::HandleCookieEcho(
    const CommonHeader& header,
    const SctpPacket::ChunkDescriptor& descriptor) {
...
bool CustomDcSctpSocket::HandleCookieEchoWithTCB(const CommonHeader& header,
...
void CustomDcSctpSocket::HandleCookieAck(
    const CommonHeader& header,
    const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::MaybeDeliverMessages() {
...
void CustomDcSctpSocket::HandleSack(const CommonHeader& header,
                              const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleHeartbeatRequest(
    const CommonHeader& header,
    const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleHeartbeatAck(
    const CommonHeader& header,
    const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleAbort(const CommonHeader& header,
                               const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleError(const CommonHeader& header,
                               const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleReconfig(
    const CommonHeader& header,
    const SctpPacket::ChunkDescriptor& descriptor) {
  Timestamp now = callbacks_.Now();
...
void CustomDcSctpSocket::HandleShutdown(
    const CommonHeader& header,
    const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleShutdownAck(
    const CommonHeader& header,
    const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleShutdownComplete(
    const CommonHeader& header,
    const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleForwardTsn(
    const CommonHeader& header,
    const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleIForwardTsn(
    const CommonHeader& header,
    const SctpPacket::ChunkDescriptor& descriptor) {
...
void CustomDcSctpSocket::HandleForwardTsnCommon(const AnyForwardTsnChunk& chunk) {
...
void CustomDcSctpSocket::MaybeSendShutdownOrAck() {
...
void CustomDcSctpSocket::SendShutdown() {
  SctpPacket::Builder b = tcb_->PacketBuilder();
  b.Add(ShutdownChunk(tcb_->data_tracker().last_cumulative_acked_tsn()));
  packet_sender_.Send(b);
}

void CustomDcSctpSocket::SendShutdownAck() {
  packet_sender_.Send(tcb_->PacketBuilder().Add(ShutdownAckChunk()));
  t2_shutdown_->set_duration(tcb_->current_rto());
  t2_shutdown_->Start();
}

HandoverReadinessStatus CustomDcSctpSocket::GetHandoverReadiness() const {
...
CustomDcSctpSocket::GetHandoverStateAndClose() {
  CallbackDeferrer::ScopedDeferrer deferrer(callbacks_);
